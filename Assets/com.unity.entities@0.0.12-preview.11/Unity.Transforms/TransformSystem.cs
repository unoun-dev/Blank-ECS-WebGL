            ...
            try
            {
                for (int length = RootLocalToWorldChunks.Length, chunkIndex = 0; chunkIndex < length; chunkIndex++)
                {
                        ...
                        for (int i = 0; i < parentCount; i++)
                        {
                            ...
                            matrix.c0.x
                                =
                                    scale.x;
                            matrix.c0.y     =
                                matrix.c0.z = matrix.c0.w =
                                                  matrix.c1.x
                                                      =
                                                          0;
                            matrix.c1.y = scale.y; matrix.c1.z =
                                matrix.c1.w
                                    =
                                        matrix
                                        .
                                        c2.x = matrix.c2.y = 0; matrix.c2.z = scale.z; matrix.c2.w =
                                matrix.c3.x
                                                        =
                                        matrix.c3.y     =
                                            matrix.c3.z = 0; matrix.c3.w = 1;
                        }
                    }
                    // 010
                    ...
                    // 011
                        ...
                        {
                            ref var scale = ref chunkScalesPtr[i];
                            chunkLocalToWorldsPtr[i].Value = math.mul(new float4x4(chunkRotations[i].Value, default(float3)), new float4x4(new float4(scale.x, 0, 0, 0), new float4(0, scale.y, 0, 0), new float4(0, 0, scale.z, 0), new float4(
                                                                                                                                               0, 0, 0, 1)));
                        }
                    }
                    // 100
                            ...
                            matrix.c1.y = scale.y; matrix.c1.z =
                                matrix.c1.w
                                    =
                                        matrix
                                        .
                                        c2.x = matrix.c2.y = 0; matrix.c2.z = scale.z; matrix.c2.w = 0;
                            matrix.c3.x
                                =
                                    position.x;
                            matrix.c3.y                 =
                                position.y; matrix.c3.z = position.z; matrix.c3.w = 1;
                        }
                    }
                    // 110
                    else if ((chunkPositionsExist) && (chunkRotationsExist) && (!chunkScalesExist)) {
